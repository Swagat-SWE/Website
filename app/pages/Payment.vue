            ...
            <div class="field">
              ...
              <div class="control">
                <input
                  ...
                  placeholder="5312 6802 9547 3321"
                  v-model="cardNumber"
...
// 🔒 Fake + garbage card detection (demo only)
const FAKE_CARDS = new Set([
  "4242424242424242",
  "4000056655665556",
  "5555555555554444",
  "2223003122003222",
...
function isGarbageNumber(digits) {
  if (!digits) return false;

  // all same digit: 111111, 222222
  if (/^(\d)\1+$/.test(digits)) return true;

  // repeating patterns: 123123, 121212
  if (/^(\d{2,6})\1+$/.test(digits)) return true;

  return false;
}

function isFakeCard(digits) {
  return FAKE_CARDS.has(digits) || isGarbageNumber(digits);
}
...
function validate() {
  ...
  const digits = onlyDigits(cardNumber.value);
  // 🔒 block fake / garbage cards
  if (isFakeCard(digits)) {
    errors.cardNumber = "Payment info failed. Fake card detected.";
    return false;
  }
